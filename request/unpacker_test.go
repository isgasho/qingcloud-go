...
func TestUnpackerUnpackHTTPRequest(t *testing.T) {
	...
	AssertNil(t, err)
	AssertEqual(t, "i-xxxxxxxx", StringValue(output.VolumeSet[0].Instance.InstanceID))
	AssertEqual(t, "vol-xxxxxxxx", StringValue(output.VolumeSet[0].VolumeID))
	AssertEqual(t, "vol name", StringValue(output.VolumeSet[0].VolumeName))
	AssertEqual(t, 1024, IntValue(output.TotalCount))
	statusTime := time.Date(2013, 8, 30, 5, 13, 32, 0, time.UTC)
	AssertEqual(t, statusTime, TimeValue(output.VolumeSet[0].StatusTime))
}
